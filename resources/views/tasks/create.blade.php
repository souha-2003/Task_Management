    ...
    <div class="row justify-content-center">
        <div class="col-md-8 col-12">
            <div class="d-flex align-items-center justify-content-between mb-4">
                <h2 class="fw-bold mb-0 text-dark">➕ {{ __('messages.create_task') }}</h2>
                    ...
                    <form method="POST" action="{{ route('tasks.store') }}">
                        ...
                        @if(auth()->user()->hasRole('admin') || auth()->user()->can('edit any task'))
                            <div class="mb-3">
                                ...
                                <input type="hidden" name="user_id" id="selected_user_id" value="{{ old('user_id', auth()->id()) }}">
                                            ...
                                            <li class="mb-2" style="list-style: none;">
                                                <button type="button" class="dropdown-item rounded-3 p-2 d-flex align-items-center gap-3 text-start user-option border" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-email="{{ $user->email }}" style="background-color: #ffffff; border-color: #e2e8f0; transition: all 0.2s ease;">
                                                    ...
                                                    <div class="d-flex flex-column align-items-start">
                                                        ...
                                                        <span class="small text-secondary" style="font-size: 0.8rem;">{{ $user->email }}</span>
                                                    </div>
                                                </button>
                                            </li>
                                        @endforeach
                                ...
                                function selectUser(id, name, email) {
                                    ...
                                    display.innerHTML = `
                                        ...
                                        <div class="d-flex flex-column align-items-start" style="line-height: 1.2;">
                                            ...
                                            <span class="small text-secondary" style="font-size: 0.75rem;">${email}</span>
                                        </div>
                                    `;
                                }
                                    ...
                                    @foreach ($categories as $category)
                                        <li class="dropdown-item rounded py-2">
                                            <div class="form-check">
                                                <input type="checkbox" name="categories[]" id="category_{{ $category->id }}" value="{{ $category->id }}" data-name="{{ $category->name }}" data-color="{{ $category->color }}" class="form-check-input category-checkbox" {{ is_array(old('categories')) && in_array($category->id, old('categories')) ? 'checked' : '' }}>
